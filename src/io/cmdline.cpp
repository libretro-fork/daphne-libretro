...
bool parse_cmd_line(int argc, char **argv)
{
	...
	// while we have stuff left in the command line to parse
	  for (;;)
	  {
        ...
        {
			// 2018.02.01 - RJS - Scoreboard isn't initialized if useoverlaysb is in command line.  While this _should_ work,
			// this shows a potential mix up during the port counting on overlay (as opposed to screen?).  So the hack here is 
			// we initialize lair/thayer with a scoreboard and clear its visibility.  This mix up needs to be unwound.

			// RJS - 2018.02.05 - I think this is hardcoding the 2nd char set for scoreboard.  Don't remeber if
			// I did this or it was that way.  Leaving for now.
			get_next_word(s, sizeof(s));
			i = atoi(s);
			i = 1;

			// 2017.09.27 - RJS - added core variable for this, did not try to wire up for mid-game changes.
			// 2018.02.05 - RJS - Thing wired up for mid-game changes.  Usually Y key.
			bool fShowOverlay = false;
			char stVar[] = "daphne_useoverlaysb";
			if (retro_get_variable(stVar) == 1)
			{
				fShowOverlay = true;
			}
			...
			if (((i == 0) && (s[0] == '0')) || (i != 0))
			{
				...
				if (NULL == game_lair_or_sa && NULL == game_thayers)
				{
					...
					if (game_lair_or_sa)
						game_lair_or_sa->init_overlay_scoreboard(fShowOverlay);
					else
						// Must be Thayer's Quest
						game_thayers->init_overlay_scoreboard(fShowOverlay);
				}
			} else {
		...
		// RJS START ADD - allow joystick inversion
		else if  (strcasecmp(s, "-invertctrl")==0)
		{
			// 2017.09.27 - RJS - added core variable for this, did not try to wire up for mid-game changes
			char stVar[] = "daphne_invertctrl";
			if (retro_get_variable(stVar) == 0)
			{
				input_invert_controls(true);
			}
		}
		// RJS END
