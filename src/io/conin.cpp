...
char con_getkey()
{
	...
	SDL_Event event;
		...
		// RJS REMOVE SDL2 is more unicode ready, this changed text input from SDL_KEYDOWN to 
		//   SDL_TEXTINPUT.  SDL_KEYDOWN is more like a 101-key joystick, you get no text but
		//   you can tell if a key was pressed.
		// SDL_EnableUNICODE(1);
			...
			if (SDL_PollEvent (&event))
			{
				...
				case SDL_KEYDOWN:
					...
					case SDLK_BACKQUOTE:
						...
						break;
					default:
						// RJS CHANGE .unicode no longer exists.  We'll try to pass the
						//   sym and see if that works (not tracing this right now).
						// result = (unsigned char) event.key.keysym.unicode;
						result = (unsigned char) event.key.keysym.sym;
						break;
					}
					break;
		...
		// RJS REMOVE unicode changed in SDL2, see above comment.
		// SDL_EnableUNICODE(0);
