...
void game::start()
{
	// RJS HERE - thread for main cpu loop here
	// cpu_execute();
	...
	if (m_game_uses_video_overlay)
	{
		...
		if ((m_video_overlay_width != 0) && (m_video_overlay_height != 0) && (m_palette_color_count != 0))
		{
			...
			for (index = 0; index < m_video_overlay_count; index++)
			{
				...
				m_video_overlay[index] = SDL_CreateRGBSurface(0, m_video_overlay_width, m_video_overlay_height, 8, 0, 0, 0, 0);
	...
	if (m_video_overlay_needs_update)
	{
		...
		if (!g_ldp->is_vldp())
		{
				...
				else
				{
					...
					// RJS START - REMOVE - hand scaling should no longer be needed, doc'd for now
					// to see results, see SDL_RenderSetScale, SDL_RenderSetLogicalSize
					/*
					...
					*/
					printline("Scaled functionality docd out here!");
					// RJS END
				} /*endelse*/
			vid_flip();
...
// RJS ADD - whole routine, just need access
bool game::does_game_use_video_overlay()
{
	return m_game_uses_video_overlay;
}

// RJS START ADD - get credits
#define MAX_CREDITS 3
static int nGameCredits[MAX_CREDITS] = { 0, 0, 0 };
int game::get_game_credits()
{
	return (nGameCredits[0] + (nGameCredits[1] * 10) + (nGameCredits[2] * 100));
}

void game::update_game_credits(unsigned int nDigit, unsigned int nValue)
{
	if (nDigit >= MAX_CREDITS) return;
	nGameCredits[nDigit] = nValue;
}
// RJS END
