...
// 2017.08.18 - RJS - I haven't pieced out how this fully works, is it better to let this run in it's
// own thread and deal with issue associated with such or just run one cycle per retro_run.  Going
// to doc out the whole routine and edit a copy.
// 2017.09.14 - RJS - Renamed this routine and made it run in it's own thread.  Currently not sure
// where frame rate is gauged.

extern int retro_run_frames;
int retro_run_frames_previous	= 0;
int retro_run_frames_delta		= 0;

// executes all cpu cores "simultaneo+*-usly".  this function only returns when the game exits
...
//*********************************************************************************************************************************
//*********************************************************************************************************************************
void cpu_execute_one_cycle_init()
{
	one_cycle_inited = 1;

	// last_inputcheck = 0; //time we last polled for input events
	cpu = g_head;
...
{
	/*
	// Making a lazy flag for init.
	if (one_cycle_inited == 0) cpu_execute_one_cycle_init();

	// You ask why, oh why this abstraction!?  I say clarity.  Stop laughing.
	cpu_execute_one_cycle();
	*/
	// RJS HERE - cpu execute loop
	cpu_execute_loop();
	return 0;
}
